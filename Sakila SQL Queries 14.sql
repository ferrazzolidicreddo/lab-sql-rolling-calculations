...
SELECT a.first_name, a.last_name, fa.actor_id, count(fa.film_id) AS Film_count, RANK() OVER(ORDER BY count(fa.film_id) DESC) AS ranking
...
CREATE VIEW actor_work_pairs AS
(select fa1.film_id, concat(a1.first_name, ' ', a1.last_name) actor_1, concat(a2.first_name, ' ', a2.last_name) actor_2
from sakila.actor a1
join film_actor fa1
on a1.actor_id = fa1.actor_id
...
-- Solution (Removing duplicates and mirrored pairs)
SELECT p1.film_id, p1.actor_1, p1.actor_2
FROM actor_work_pairs AS p1 
INNER JOIN actor_work_pairs AS p2 
    on p1.actor_1 = p2.actor_2 
    and p1.actor_2 = p2.actor_1 
    and p1.film_id = p2.film_id
    -- match only one of the two pairs
    and p1.actor_1 > p1.actor_2;

-- For each film, list actor that has acted in more films.
SELECT * FROM sakila.film_actor;

# Ranking the most prolific actor
SELECT a.first_name, a.last_name, fa.actor_id, count(fa.film_id) AS Film_count, RANK() OVER(ORDER BY count(fa.film_id) DESC) AS ranking
FROM sakila.film_actor AS fa
JOIN sakila.actor AS a
ON fa.actor_id = a.actor_id
...
# Creating the actors ranking
CREATE VIEW Actors_ranking AS
(SELECT a.first_name, a.last_name, fa.actor_id, count(fa.film_id) AS Film_count, RANK() OVER(ORDER BY count(fa.film_id) DESC) AS ranking
FROM sakila.film_actor AS fa
JOIN sakila.actor AS a
ON fa.actor_id = a.actor_id
...
WITH Actors_ranking AS (SELECT a.first_name, a.last_name, fa.actor_id, count(fa.film_id) AS Film_count, RANK() OVER(ORDER BY count(fa.film_id) DESC) AS ranking
FROM sakila.film_actor AS fa
JOIN sakila.actor AS a
ON fa.actor_id = a.actor_id
GROUP BY actor_id
ORDER BY Film_count DESC)
SELECT * FROM Actors_ranking AS ar2
JOIN sakila.film_actor as fa2
ON ar2.actor_id = fa2.actor_id
GROUP BY film_id, fa2.actor_id
HAVING min(ranking);

CREATE VIEW Actors_ranking_2 AS
(SELECT ar2.first_name, ar2.last_name, ar2.actor_id, ar2.Film_count, ar2.ranking, fa2.film_id FROM Actors_ranking AS ar2
JOIN sakila.film_actor as fa2
ON ar2.actor_id = fa2.actor_id
GROUP BY film_id, ranking
HAVING min(ranking));

# Ranking the most prolific actor per each film (Solution)
SELECT film_id AS Film, Film_count AS 'Film count', actor_id AS 'Actor id', ranking AS 'Ranking',
first_name AS 'First Name', last_name AS 'Last Name' FROM Actors_ranking_2
GROUP BY film_id
ORDER BY film_id, ranking ASC;
...
# Finding active users per Month
SELECT YEAR(rental_date) AS Year, MONTH(rental_date) AS Month, count(distinct customer_id) AS Active_users FROM sakila.rental
GROUP BY Month;

-- Active users in the previous month (August).
SELECT YEAR(rental_date) AS Year, MONTH(rental_date) AS Month, count(distinct customer_id) AS Active_users FROM sakila.rental
GROUP BY Month
HAVING Month = MONTH('2005-08-23 22:50:12');

-- Percentage change in the number of active customers.
#Overview
SELECT * FROM sakila.rental;

-- Alternative Approach (Based on the Bank model) --
...
# Creating a view of the Monthly_active_users
...
# Creating a view for retained_customers
...
from sakila.distinct_users d1
join sakila.distinct_users d2 on d1.customer_id = d2.customer_id
